# Invincat CLI 记忆治理设计全景：从可用助手到可治理的长期工程协作系统
...
第一，完善自动化运行文档，覆盖 `--non-interactive`、`--acp`、CI 场景和 shell allow-list。第二，补齐 async subagents 与 sandbox providers 的配置指南，让远程任务和隔离执行更容易落地。第三，增强记忆迁移工具，将旧 `AGENTS.md` 或历史配置迁移到 JSON 记忆存储（Memory Store）。第四，建立 docs-check 机制，让文档持续跟随代码演进。
