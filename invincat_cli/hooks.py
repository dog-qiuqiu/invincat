...
Loads hook configuration from `~/.invincat/hooks.json` and fires matching
...
Config format (`~/.invincat/hooks.json`):
...
def _load_hooks() -> list[dict[str, Any]]:
    ...
    from invincat_cli.model_config import DEFAULT_CONFIG_DIR
