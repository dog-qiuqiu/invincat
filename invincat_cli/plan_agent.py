...
def build_plan_directive(task: str) -> str:
    ...
    return (
        ...
        f"Task: {task}\n\n"
        f"If the planner returns a message starting with "
        f"`{PLAN_APPROVED_MARKER}`, proceed to implement every todo in order. "
        f"If it returns anything else (e.g. cancellation), stop and tell me."
    )
