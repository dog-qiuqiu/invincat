"""Command-line argument parser construction for the Invincat CLI."""
...
def _lazy_help(fn_name: str) -> Callable[[], None]:
    """Return a help callback that imports the Rich UI only when invoked."""
...
def _help_parent(help_fn: Callable[[], None]) -> list[argparse.ArgumentParser]:
    """Build a parent parser that routes `-h/--help` to a custom help screen."""
    parent = argparse.ArgumentParser(add_help=False)
    parent.add_argument("-h", "--help", action=_make_help_action(help_fn))
    return [parent]


def _add_thread_commands(subparsers: argparse._SubParsersAction[Any]) -> None:
    """Register thread management subcommands."""
    ...
    threads_parser = subparsers.add_parser(
        ...
        add_help=False,
        parents=_help_parent(_lazy_help("show_threads_help")),
    )
    ...
    threads_list = threads_sub.add_parser(
        ...
        help="List threads",
        add_help=False,
        parents=_help_parent(_lazy_help("show_threads_list_help")),
    )
    ...
    threads_delete = threads_sub.add_parser(
        ...
        add_help=False,
        parents=_help_parent(_lazy_help("show_threads_delete_help")),
    )
...
def _add_management_commands(subparsers: argparse._SubParsersAction[Any]) -> None:
    """Register non-session management subcommands."""
    from invincat_cli.io.output import add_json_output_arg
    ...
    subparsers.add_parser(
        ...
        add_help=False,
        parents=_help_parent(_lazy_help("show_help")),
    )
    ...
    agents_parser = subparsers.add_parser(
        ...
        help="Manage agents",
        add_help=False,
        parents=_help_parent(_lazy_help("show_agents_help")),
    )
    ...
    agents_list = agents_sub.add_parser(
        ...
        add_help=False,
        parents=_help_parent(_lazy_help("show_list_help")),
    )
    ...
    _add_thread_commands(subparsers)

    update_parser = subparsers.add_parser(
        "update",
        help="Check for and install CLI updates",
        add_help=False,
        parents=_help_parent(_lazy_help("show_update_help")),
    )
...
def _add_model_args(parser: argparse.ArgumentParser) -> None:
    """Register model selection and persistence flags."""
    parser.add_argument(
        "-M",
        "--model",
        metavar="MODEL",
...
def _add_session_args(parser: argparse.ArgumentParser, *, default_agent_name: str) -> None:
    """Register interactive and non-interactive session flags."""
    parser.add_argument(
        "-r",
        "--resume",
        dest="resume_thread",
...
def _add_execution_args(parser: argparse.ArgumentParser) -> None:
    """Register tool execution, sandbox, and MCP flags."""
    parser.add_argument(
        "-y",
        "--auto-approve",
        action="store_true",
...
def _sdk_version() -> str:
    """Return the installed deepagents SDK version for `--version` output."""
    try:
        from importlib.metadata import PackageNotFoundError
        from importlib.metadata import version as _pkg_version

        return _pkg_version("deepagents")
...
def parse_args(*, default_agent_name: str = "agent") -> argparse.Namespace:
    ...
    _add_management_commands(subparsers)

    # Argument order here determines argparse's usage line; keep in sync with
    # the custom Rich help rendered by ui.show_help().
    _add_session_args(parser, default_agent_name=default_agent_name)
