...
"""Directory for user-level Deep Agents configuration (`~/.invincat`)."""

DEFAULT_CONFIG_PATH = DEFAULT_CONFIG_DIR / "config.toml"
"""Path to the user's model configuration file (`~/.invincat/config.toml`)."""
...
def _save_model_field(
    ...
    Args:
        ...
        config_path: Path to config file.
            ...
            Defaults to `~/.invincat/config.toml`.
...
def register_provider_model(
    ...
    *,
    ...
    Reads the existing config (if any), adds or updates the model entry
    under ``[models.providers.<provider_name>]``, and writes back using
    atomic temp-file rename.

    If the provider already exists, the model is appended to its ``models``
    list (deduplicated).  ``class_path`` is only set when the provider is
    first created.  ``base_url`` and ``api_key_env`` are stored in
    ``params.<model_name>`` to allow per-model configuration.
    ``max_input_tokens`` is stored in ``profile.<model_name>``.
    ...
    Args:
        ...
        api_key_env: Environment variable name for the API key
            (e.g. ``'DEEPSEEK_API_KEY'``).  Stored in
            ``params.<model_name>.api_key_env`` for per-model configuration.
        base_url: Custom base URL for this model's API endpoint.
            Stored in ``params.<model_name>.base_url`` for per-model
            configuration.
        ...
        config_path: Path to config file.  Defaults to
            ``~/.invincat/config.toml``.

    Returns:
        ``True`` if save succeeded, ``False`` if it failed due to I/O
        errors.
    """
    if config_path is None:
        config_path = DEFAULT_CONFIG_PATH
    ...
    try:
        ...
        # Add model to the provider's models list (deduplicate)
        models_list: list[str] = provider_cfg.get("models", [])
        if model_name not in models_list:
            models_list.append(model_name)
        provider_cfg["models"] = models_list

        # Only set class_path when creating a new provider section
