...
TRANSLATIONS: dict[str, dict[str, str]] = {
    Language.EN: {
        ...
        "command.plan": "/plan <task> delegates to the planner subagent; bare /plan toggles plan mode",
        "command.exit_plan": "Exit plan mode and resume normal execution",
        "plan.entered": "Plan mode is ON. Write tools (write_file, edit_file, execute, task, async-subagent, compact) will be rejected. Run /exit-plan when ready to implement.",
        "plan.exited": "Plan mode is OFF. Write tools are re-enabled.",
        "plan.already_on": "Plan mode is already active. Run /exit-plan to leave it.",
        "plan.already_off": "Plan mode is not active.",
        "plan.tool_blocked": "Plan mode blocked tool: {tool}. The agent should keep refining the plan.",
        "plan.usage": "Usage: /plan <task description>, or bare /plan to toggle plan mode.",
        "approve.prompt": "Press 'y' to approve and execute, or 'n' to refine the plan",
        "approve.approved": "Plan approved. Starting execution...",
    ...
    Language.ZH: {
        ...
        "command.plan": "/plan <任务> 交由 planner 子智能体规划；裸 /plan 则切换计划模式",
        "command.exit_plan": "退出计划模式，恢复正常执行",
        "plan.entered": "已进入计划模式。写入类工具 (write_file、edit_file、execute、task、async-subagent、compact) 将被自动拒绝。准备实施时请运行 /exit-plan。",
        "plan.exited": "已退出计划模式，写入类工具已恢复。",
        "plan.already_on": "计划模式已激活，运行 /exit-plan 可退出。",
        "plan.already_off": "当前未处于计划模式。",
        "plan.tool_blocked": "计划模式拦截了工具：{tool}。智能体应继续完善方案。",
        "plan.usage": "用法：/plan <任务描述>；或输入裸 /plan 以切换计划模式。",
        "approve.prompt": "按 'y' 确认并执行，或按 'n' 完善计划",
        "approve.approved": "计划已确认，开始执行...",
