...
def _load_dotenv(*, start_path: Path | None = None) -> bool:
    ...
    2. `~/.invincat/.env` — global user defaults
    ...
    # 2. Global (~/.invincat/.env) — fills in any vars not already set by
...
class Settings:
    ...
    def get_agent_dir(self, agent_name: str) -> Path:
        ...
        if not self._is_valid_agent_name(agent_name):
            ...
            raise ValueError(msg)
        return Path.home() / ".invincat" / agent_name
...
def _get_provider_kwargs(
    ...
    sub-table are shallow-merged on top. Model-level `base_url` in params
    takes precedence over provider-level `base_url`.
    ...
    # Model-level base_url (from params.<model_name>.base_url) takes precedence
    # over provider-level base_url. If result already has base_url from params,
    # don't overwrite it.
    ...
    from invincat_cli.model_config import PROVIDER_API_KEY_ENV, resolve_env_var

    # Model-level api_key_env (from params.<model_name>.api_key_env) takes
    # precedence over provider-level. If result already has api_key directly,
    # skip env var resolution.
