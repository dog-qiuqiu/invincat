...
class ThemeSelectorScreen(ModalScreen[str | None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        for i, name in enumerate(sorted(theme.ThemeEntry.REGISTRY.keys())):
            entry = theme.ThemeEntry.REGISTRY[name]
            label = entry.label
            if name == self._current_theme:
                label = f"{label} {t('theme.current')}"
                highlight_index = i
            options.append(Option(label, id=name))

        with Vertical():
            yield Static(t("theme.select_title"), classes="theme-selector-title")
            option_list = OptionList(*options, id="theme-options")
            option_list.highlighted = highlight_index
            yield option_list
            help_text = (
                f"{glyphs.arrow_up}/{glyphs.arrow_down} {t('theme.preview')}"
                f" {glyphs.bullet} Enter {t('theme.select')}"
                f" {glyphs.bullet} Esc {t('theme.cancel')}"
            )
