...
class ApprovalMenu(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        if self._security_warnings:
            parts: list[Content] = [
                Content.from_markup(
                    f"[yellow]{t('approval.warning_deceptive')}[/yellow]"
                ),
            ]
            parts.extend(
                Content.from_markup("\n[dim]- $w[/dim]", w=warning)
        ...
        glyphs = get_glyphs()
        help_text = (
            f"{glyphs.arrow_up}/{glyphs.arrow_down} {t('language.preview')} {glyphs.bullet} "
            f"Enter {t('language.select')} {glyphs.bullet} y/a/n {glyphs.bullet} Esc {t('language.cancel')}"
        )
        if self._has_expandable_command:
            help_text += f" {glyphs.bullet} e {t('approval_menu.expand_short')}"
