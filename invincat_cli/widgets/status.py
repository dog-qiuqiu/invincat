"""Status bar widget for invincat-cli."""
...
class StatusBar(Horizontal):
    ...
    def watch_mode(self, mode: str) -> None:
        ...
        if mode == "shell":
            indicator.update(t("status.shell_mode"))
            indicator.add_class("shell")
        elif mode == "command":
            indicator.update(t("status.cmd_mode"))
            indicator.add_class("command")
        else:
            indicator.update("")
            indicator.add_class("normal")

    def watch_plan_mode(self, new_value: bool) -> None:
        """Update plan-mode pill when state changes."""
        try:
    ...
    def watch_auto_approve(self, new_value: bool) -> None:
        ...
        if new_value:
            indicator.update(f"{t('approval.auto_approve')} | shift+tab")
            indicator.add_class("on")
        else:
            indicator.update(f"{t('approval.approve')} | shift+tab")
    ...
    def watch_message_count(self, new_value: int) -> None:
        """Update message count display."""
        try:
            display = self.query_one("#message-count-display", Static)
        except NoMatches:
            return
    ...
    def _render_tokens(self, count: int, *, approximate: bool = False) -> None:
        ...
        Color-codes the display based on context window usage:
        - < 70 %  → muted (normal)
        ...
        Args:
            ...
            approximate: Append "~" prefix to indicate the count is stale
