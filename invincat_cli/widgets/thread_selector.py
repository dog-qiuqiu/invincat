...
class DeleteThreadConfirmScreen(ModalScreen[bool]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="delete-confirm"):
            yield Static(
                Content.from_markup(
                    t("thread.delete_confirm", thread_id=self._delete_thread_id),
                    tid=self._delete_thread_id,
                ),
                classes="thread-confirm-text",
            )
            yield Static(
                t("thread.delete_help"),
...
class ThreadSelectorScreen(ModalScreen[str | None]):
    ...
    def _build_title(self, thread_url: str | None = None) -> str | Content:
        ...
        if thread_url:
            return Content.assemble(
                f"{t('thread.title')} ({t('thread.current_thread', thread_id='')}",
                (
        ...
        return f"{t('thread.title')} ({t('thread.current_thread', thread_id=self._current_thread)})"
    ...
    def _build_help_text(self) -> str:
        ...
        glyphs = get_glyphs()
        lines = (
            f"{glyphs.arrow_up}/{glyphs.arrow_down} {t('thread.navigate')}"
            f" {glyphs.bullet} Enter {t('thread.select_action')}"
            f" {glyphs.bullet} Tab/Shift+Tab {t('thread.focus_options')}"
            f" {glyphs.bullet} Space {t('thread.toggle_option')}"
            f" {glyphs.bullet} Ctrl+D {t('thread.delete_action')}"
            f" {glyphs.bullet} Esc {t('thread.cancel_action')}"
        )
        limit = self._effective_thread_limit()
        if len(self._threads) >= limit:
            lines += "\n" + t("thread.showing_limit", limit=limit)
        return lines
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="thread-selector-shell"):
            ...
            yield Input(
                placeholder=t("thread.filter_placeholder"),
    ...
    def _persist_sort_order(self, order: str) -> None:
        ...
        async def _save() -> None:
            ...
            if not ok:
                self.app.notify(t("thread.sort_save_failed"), severity="warning")
    ...
    async def _handle_delete_confirm(self, thread_id: str) -> None:
        ...
        try:
            ...
            self.app.notify(
                t("thread.delete_failed", thread_id=thread_id[:8]),
                severity="error",
                timeout=3,
                markup=False,
            )
