...
class AskUserMenu(Container):
    ...
    def compose(self) -> ComposeResult:  # noqa: D102
        ...
        yield Static("")
        parts = [
            f"{glyphs.arrow_up}/{glyphs.arrow_down} {t('ask.select')}",
            t("ask.enter_continue"),
        ]
        if len(self._questions) > 1:
            parts.append(t("ask.switch_question"))
        parts.append(t("ask.cancel"))
        yield Static(
