...
class EditFileApprovalWidget(ToolApprovalWidget):
    ...
    def compose(self) -> ComposeResult:
        ...
        if not diff_lines and not old_string and not new_string:
            yield Static(t("tool.no_changes"), classes="approval-description")
        elif diff_lines:
            # Render content
    ...
    def _render_diff_lines_only(self, diff_lines: list[str]) -> ComposeResult:
        ...
        for line in diff_lines:
            if lines_shown >= _MAX_DIFF_LINES:
                yield Static(
                    Content.styled(
                        t("tool.more_lines", count=len(diff_lines) - lines_shown), "dim"
                    )
                )
                break
    ...
    def _render_strings_only(self, old_string: str, new_string: str) -> ComposeResult:
        ...
        colors = theme.get_theme_colors()
        if old_string:
            yield Static(Content.styled(t("tool.removing"), f"bold {colors.error}"))
            yield from self._render_string_lines(old_string, is_addition=False)
            yield Static("")

        if new_string:
            yield Static(Content.styled(t("tool.adding"), f"bold {colors.success}"))
