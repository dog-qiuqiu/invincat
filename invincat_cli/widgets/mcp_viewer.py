...
class MCPViewerScreen(ModalScreen[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical():
            if total_servers:
                title = (
                    f"{t('mcp.title')} ({t('mcp.servers_count', count=total_servers)},"
                    f" {t('mcp.tools_count', count=total_tools)})"
                )
            else:
                title = t("mcp.title")
                    ...
                    for server in self._server_info:
                        ...
                        yield Static(
                            Content.from_markup(
                                ...
                                f" {t('mcp.tools_count', count=tool_count)}[/dim]",
                                name=server.name,
            ...
            help_text = (
                f"{glyphs.arrow_up}/{glyphs.arrow_down} {t('mcp.navigate')}"
                f" {glyphs.bullet} Enter {t('mcp.expand_collapse')}"
                f" {glyphs.bullet} Esc {t('mcp.close')}"
            )
